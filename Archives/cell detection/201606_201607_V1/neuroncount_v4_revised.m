function [neurons,processes]=neuroncount_v4_revised(Bcrop,colorchannel)
%% 1. Preprocessing
...
Bcrop=255./(1+exp(-(double(Bcrop)-240)/30));
...
%
% 1.2 Gaussian blur
Bfilt = imfilter(Bcrop,fspecial('gaussian',2*ceil(2*10)+1, 10));
% Bfilt=imgaussfilt(Bnorm,10);
Bcrop = Bcrop - Bfilt; % remove the blurred image to get sharp image
...
bw=im2bw(Bcrop,graythresh(Bcrop)); % convert to binary image using Otsu's threshold
