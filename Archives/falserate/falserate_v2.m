    ...
    for v=1:win.vert % then move down
        for h=1:win.hori % first move horizontally
            ...
            if (sum(sum(imgtemp_mono>100))>10) % if there are at least 10 pixels exceed the threshold of 100
                ...
                hold on
                % find the x and y coordinates of cell bodies within the
                % current window
                ...
                groundmask=groundmaskx.*groundmasky; % both x and y coordinates need to be inside the window
                ...
                if (sum(sum(imgtemp_rgb(:,:,3)==255))>100)
                    ...
                    imgtemp_mono=imgsat+imgtemp_unsat;
                ...
                title('Please select from this panel','fontsize',18)
                hold on
                %             for i=1:length(ind)
                %                 scatter(groundtruth{i}.x(ind(i))-(h-1)*win.width,groundtruth{i}.y(ind(i))-(v-1)*win.height,'m*')
                %             end
