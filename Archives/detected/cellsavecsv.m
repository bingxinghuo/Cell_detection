celloc=[];
for i=1:337
load(['cellmask_',num2str(f),'.mat'])
cc=bwconncomp(savedata);
%% file 1: cell locations
[x,y]=find(savedata);
        celloc=[celloc;[i*ones(length(FBclear{i}.x),1),FBclear{i}.x,FBclear{i}.y]];
%     elseif bitinfo==12
%         celloc=[celloc;[i*ones(length(FBdetected{i}.x),1),FBdetected{i}.x,FBdetected{i}.y]];
%     end
end
celloc=round(celloc);
