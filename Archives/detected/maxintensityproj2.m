...
%%
load meanproj
fluoroimg_sat=zeros(size(fluoroimg3d));
for f=1:size(fluoroimg3d,3)
    fluoroimg=squeeze(fluoroimg3d(:,:,f,:));
    fluoroimg1(:,:,1)=fluoroimg(:,:,1)-max(fluoroimg(:,:,2),fluoroimg(:,:,3));
    fluoroimg1(:,:,2)=fluoroimg(:,:,2)-max(fluoroimg(:,:,1),fluoroimg(:,:,3));
    fluoroimg1(:,:,3)=fluoroimg(:,:,3)-max(fluoroimg(:,:,1),fluoroimg(:,:,2));
    fluoroimg_sat(:,:,:,f)=fluoroimgimg1;
end
maxint.x=max(fluoroimg_sat,[],1);
maxint.y=max(fluoroimg_sat,[],2);
maxint.z=max(fluoroimg_sat,[],4);
save('maxproj','fluoroimg_sat','maxint','-v7.3')
% 
% % initialization
