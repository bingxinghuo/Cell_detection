function [bwimg_patch,and,localmax]=cellpatch1(bwimg,pixintimg)
global  sigma
%% initialize
...
% 1.3. Combine the mask, distance matrix and intensity matrix
dist_tr_norm=dist_tr/(max(max(dist_tr)));
pixintimg_norm=pixintimg/(max(max(pixintimg)));
and = bwimg1.*dist_tr_norm.*pixintimg_norm; % reconstruct
and(isnan(and))=0;
