function FBdetect_svmbloc(rgbimg,n,svmmodel,fileid)
global bitinfo sigma sizepar eccpar
myfun = @(block_struct) FBdetect_svmfun(block_struct,svmmodel);
block_size=[512,512];
border_size=[n,n];
fileout=[fileid(1:end-4),'_cells.jp2'];
blockproc(rgbimg,block_size,myfun,'BorderSize',border_size,...
    'PadMethod','replicate','Destination',fileout,'UseParallel',true);
end

function FBcellmask=FBdetect_svmfun(blockstruct,svmmodel)
global bitinfo sigma sizepar eccpar
rgbimg=blockstruct.data;
...
if sum(sum(rgbimg(:,:,3)>50))<100
    FBcellmask=uint8(zeros(rows,cols));
else
