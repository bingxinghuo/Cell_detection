...
% and green
red_ind=pixangles(:,1)<min(.1,min(pixangles(:,2:3),[],2));
% red_ind=red_ind.*(pixangles(:,1)<.1);
red_mask=reshape(red_ind,rows,cols);
%% combine the two
groupmask=groupimg.*red_mask;
% 3. Operate on the grayscale mask
se=strel('disk',3);
groupmask=imclose(groupmask,se);
% groupmask=imfill(groupmask,'holes'); % fill holes
groupmask1=bwareaopen(groupmask,3); % remove small connected areas (one cell size)
