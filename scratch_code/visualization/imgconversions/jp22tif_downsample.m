...
for d=1:2
    ...
    parfor f=1:Nf
        ...
        if ~exist([pwd,'/',fileout])
            ...
            % 4. downsample
            tic;
            [rows,cols]=size(maskimg);
            padrow=M-mod(rows,M);
            maskimgpad=[maskimg;uint8(zeros(padrow,cols))];
            padcol=M-mod(cols,M);
            maskimgpad=[maskimgpad,uint8(zeros(rows+padrow,padcol))];
            [rows,cols]=size(maskimgpad);
            maskdown=uint8(zeros(rows/M,cols/M));
            for rs=1:rows/M
                for cs=1:cols/M
                    imgblk=maskimgpad((rs-1)*M+1:rs*M,(cs-1)*M+1:cs*M);
                    maskdown(rs,cs)=max(reshape(imgblk,M^2,1));
                end
            end
            toc
            %         end
