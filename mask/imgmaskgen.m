function imgmask=imgmaskgen(fluoroimg1,sigma,maskarea)
% sigma=1;
...
if ~isempty(Ncross)
    L=0;
    t=0;
    while sum(sum(L>0))<=maskarea/2 % smaller than 50% of the previous mask
        t=t+1;
        if t<=length(Ncross)
            thresh=x(Ncross(t));
    ...
    totpix=rows*columns;
    [num1,~]=find(num>max(totpix*.0001)); % the object is at least 0.1% of the whole image
